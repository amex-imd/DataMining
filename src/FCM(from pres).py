    ...
    def evalCenter(self, PP, U):
        for d in range(DIM_NUM):
            ...
            for pi in range(len(PP)):
                termUp += U[pi][self.clustNum] * PP[pi].coords[d]
                termDown += U[pi][self.clustNum]
...
def evalMrxU(CC, PP):
    ...
    for i in range(len(PP)):
        ...
        for k in range(len(CC)):
            d = CC[k].EuclideanDistance(PP[i])
            if d == 0:
                for j in range(len(CC)):
                    res[i].append(1.0 if j == k else 0.0)
                break
            values.append(1 / pow(d, 1/Q))
        else:
            a = sum(values)
            for val in values:
                res[i].append(val / a)
